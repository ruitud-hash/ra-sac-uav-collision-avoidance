"""Fail-closed integrity audit for the 30-cell Batch-5 full-shield extension."""
...
ALL_METHODS = ("RA-SAC-v3.2-c", "SAC-Attention-v3", *METHODS)
...
def audit(args: argparse.Namespace) -> tuple[dict, dict]:
    ...
    assert manifest["status"] == "FROZEN_BEFORE_EXECUTION" and manifest["experiment"] == "batch5"
    assert manifest["cell_count"] == 30 and manifest["episode_count"] == 6000
    assert manifest["original_protocol_cells"] == 15 and manifest["post_opening_extension_cells"] == 15
    ...
    assert launcher["status"] == "COMPLETE" and len(launcher["cells"]) == 30
    ...
    assert len(launch_by_id) == 30 and set(launch_by_id) == {item["cell_id"] for item in manifest["cells"]}

    expected = {(method, environment, seed) for method in METHODS for environment in ENVIRONMENTS for seed in SEEDS}
    planned = {(item["method"], item["environment"], int(item["training_seed"])): item for item in manifest["cells"]}
    assert set(planned) == expected
    assert Counter(item["authorization"] for item in manifest["cells"]) == {
        "original_frozen_protocol": 15,
        "post_opening_prospective_extension": 15,
    }

    complete, scenarios = {}, {}
    for summary_path in args.input_root.glob("*/tables/summary.yaml"):
        summary = yaml.safe_load(summary_path.read_text(encoding="utf-8"))
        environment = next(name for name in ENVIRONMENTS if inventory["environments"][name]["sha256"] == summary["environment_config_sha256"])
        key = (summary["method"], environment, int(summary["training_seed"]))
        assert key in expected and key not in complete
        item = planned[key]
        assert summary["evaluation_mode"] == "full_shield"
        ...
        assert summary["checkpoint_sha256"] == item["checkpoint_sha256"]
        assert summary["training_config_sha256"] == item["training_config_sha256"]
        assert summary["training_metadata_sha256"] == item["training_metadata_sha256"]
        assert summary["environment_config_sha256"] == item["environment_sha256"]
        assert {name: bool(summary["action_safety_filter"][name]) for name in ("enabled", "static_boundary_enabled", "dynamic_enabled")} == {
            "enabled": True, "static_boundary_enabled": True, "dynamic_enabled": True,
        }
        assert summary["shield_metrics_applicable"] is True and finite_tree(summary)

        episodes_path = summary_path.with_name("episodes.csv")
        rows = episode_rows(episodes_path)
        ...
        for row in rows:
            ...
            assert int(row["safety_filter_modifications"]) == int(row["shield_intervention_count"])
            total_modifications += int(row["shield_intervention_count"])
            total_steps += int(row["steps"])
            correction_sum += float(row["safety_filter_correction_sum"])
        ...
        assert summary["total_safety_filter_modifications"] == total_modifications
        assert math.isclose(summary["shield_intervention_frequency"], total_modifications / 200)
        assert math.isclose(summary["shield_intervention_rate"], total_modifications / total_steps)
        assert math.isclose(summary["shield_correction_magnitude"], correction_sum / max(total_modifications, 1))
        ...
        complete[key] = {
            "method": key[0], "environment": key[1], "training_seed": key[2], "mode": "full_shield",
            "authorization": item["authorization"], "directory": directory.relative_to(ROOT).as_posix(),
    ...
    assert set(complete) == expected and len(list(args.input_root.iterdir())) == 30
    assert len(list(args.logs.glob("*.stdout.log"))) == 30 and len(list(args.logs.glob("*.stderr.log"))) == 30

    reference_paths = {
        "batch1_policy": ROOT / "outputs/publication_confirmation_v1/formal_freeze/authorized_scientific_cells.yaml",
        "batch3_shield": ROOT / "outputs/publication_confirmation_v1/formal_freeze/batch3_authorized_scientific_cells.yaml",
        "batch4_policy": ROOT / "outputs/publication_confirmation_v1/formal_freeze/batch4_authorized_scientific_cells.yaml",
    }
    reference_cells = []
    for source, path in reference_paths.items():
        for cell in load_allowlist(path):
            mode = cell.get("mode", "policy_only")
            if cell["environment"] in ENVIRONMENTS and (
                source == "batch1_policy" or source == "batch4_policy" or mode == "full_shield"
            ):
                reference_cells.append({**cell, "mode": mode, "source": source})
    assert Counter(cell["source"] for cell in reference_cells) == {
        "batch1_policy": 20, "batch3_shield": 20, "batch4_policy": 30,
    }

    deployment_keys = {(key[0], key[1], key[2], "full_shield") for key in complete}
    for cell in reference_cells:
        key = (cell["method"], cell["environment"], int(cell["training_seed"]), cell["mode"])
        assert key not in deployment_keys
        ...
        for row in episode_rows(episodes_path):
            assert scenarios[(cell["environment"], int(row["seed"]))] == json.loads(row["episode_randomization"])
    expected_deployment = {
        (method, environment, seed, mode)
        for method in ALL_METHODS for environment in ENVIRONMENTS for seed in SEEDS for mode in ("policy_only", "full_shield")
    }
    assert deployment_keys == expected_deployment

    allowlist = {
        "schema_version": 1, "created_at": datetime.now(timezone.utc).isoformat(), "status": "FROZEN",
        "purpose": "exclusive Batch-5 broad-baseline full-shield scientific-analysis allowlist",
        "cell_count": 30, "reference_cell_count": 70, "complete_deployment_cell_count": 100,
        "cells": [complete[key] for key in sorted(complete)],
    }
    record = {
        "schema_version": 1, "status": "PASS", "audit_role": "Batch-5 broad-baseline full-shield integrity and authorization",
        "performance_interpretation_performed": False, "new_cell_count": 30, "new_episode_count": 6000,
        "reference_cells": 70, "complete_deployment_cells": 100,
        "checks": {
            "logical_matrix_exact": True, "episodes_and_seed_coverage_exact": True,
            "checkpoint_environment_and_provenance_hashes_match": True,
            "full_shield_flags_enabled": True, "shield_telemetry_summary_matches_episodes": True,
            "terminal_outcomes_conserve": True, "collision_and_safety_identities_match": True,
            "five_method_scenario_randomization_match": True, "nan_or_inf_count": 0,
            "nonempty_stderr_count": 0, "launcher_complete_cells": 30,
        },
        "limitations": [
            "The 15 Combined-OOD cells are a post-opening prospective extension.",
            "Full-shield telemetry supports total intervention burden only, not static/dynamic source attribution.",
            "Breadth baselines support deployment hierarchy and trade-off context, not deep causal mechanism claims.",
        ],
        ...
        "reference_allowlist_sha256": {name: sha256(path) for name, path in reference_paths.items()},
    }
    return record, allowlist


def self_test() -> None:
    assert len({(m, e, s) for m in METHODS for e in ENVIRONMENTS for s in SEEDS}) == 30
    assert len({(m, e, s, mode) for m in ALL_METHODS for e in ENVIRONMENTS for s in SEEDS for mode in ("policy_only", "full_shield")}) == 100
...
def main() -> None:
    ...
    print("BATCH5 INTEGRITY AUDIT: PASS (30 cells, 6000 episodes; 100-cell deployment population)")
